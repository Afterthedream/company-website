...
const SolutionCard = React.memo(({ solution, index, isVisible, hoveredIndex }: {
...
export default function Solutions() {
  ...
  const displaySolutions = useMemo(() => {
    return solutions.slice(0, 3).map((solution: any, index: number) => ({
      ...
      slug: solution.slug || slugify(solution.title || defaultSolutions[index]?.title || ''),
  ...
  return (
    <section 
      ref={sectionRef} 
      className="py-28 bg-white dark:bg-surface-900 relative overflow-hidden"
      aria-labelledby="solutions-section-title"
    >
      {/* 背景装饰 */}
      <div className="absolute top-32 right-0 w-72 h-72 bg-primary-50/50 dark:bg-primary-900/20 rounded-full blur-[100px]" aria-hidden="true" />
      <div className="absolute bottom-32 left-0 w-56 h-56 bg-accent-50/40 dark:bg-accent-900/20 rounded-full blur-[80px]" aria-hidden="true" />
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <div className="w-16 h-16 mx-auto mb-5 bg-surface-100 rounded-full flex items-center justify-center">
              <svg className="w-8 h-8 text-error-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">
