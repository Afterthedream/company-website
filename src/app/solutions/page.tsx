      ...
      <section className="py-24 bg-surface-50 dark:bg-surface-950">
        <div className="max-w-6xl mx-auto px-6">
          ...
          <div className="flex items-center gap-6 mb-16">
            ...
            <span className="text-xs font-bold tracking-widest uppercase text-surface-300 dark:text-surface-500">SOLUTIONS</span>
            <div className="h-px flex-1 bg-surface-100 dark:bg-surface-800" />
          </div>
          ...
          {loading ? (
            ...
            <div>
              {displaySolutions.map((solution: any, index: number) => {
                ...
                let featuresArray: string[] = parsedFeatures.length > 0
                  ? parsedFeatures.map(f => f.label)
                  : (defaultSolutions[index]?.features || [])
                ...
                return (
                  ...
                  >
                    {/* Top rule */}
                    <div className={`h-px w-full transition-colors duration-300 ${isExpanded ? 'bg-primary-300 dark:bg-primary-700' : 'bg-surface-100 dark:bg-surface-800'}`} />
                      ...
                      >
                        <div className="grid grid-cols-[3rem_1fr_auto] md:grid-cols-[5rem_1fr_auto] items-center gap-6 md:gap-10 py-8 md:py-10">
                          ...
                          <div className="self-start pt-1">
                            <span
                              className={`font-display text-2xl md:text-3xl font-bold leading-none select-none tabular-nums transition-colors duration-300 ${isExpanded ? 'text-primary-600 dark:text-primary-400' : 'text-surface-200 dark:text-surface-600'}`}
                            >
                          ...
                          <div className="space-y-1.5 min-w-0">
                            <h2
                              className="font-display text-xl md:text-2xl font-bold tracking-tight leading-snug text-surface-900 dark:text-surface-100 group-hover:text-primary-700 dark:group-hover:text-primary-400 transition-colors duration-200"
                            >
                              {solution.title || defaults.title}
                            </h2>
                            <p className="text-sm text-surface-500 dark:text-surface-400 leading-relaxed line-clamp-1 md:line-clamp-none">
                              {desc}
                            </p>
                          </div>
                            ...
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center transition-all duration-300 ${
                                isExpanded
                                  ...
                                  : 'bg-surface-100 dark:bg-surface-800 text-surface-500 dark:text-surface-400 group-hover:bg-primary-50 dark:group-hover:bg-primary-900 group-hover:text-primary-500 dark:group-hover:text-primary-400'
                              }`}
                            >
                              <svg
                          ...
                          <div className="grid md:grid-cols-[5rem_1fr_minmax(0,20rem)] gap-6 md:gap-10 pb-10">
                            ...
                            <div className="space-y-6">
                              ...
                              {featuresArray.length > 0 && (
                                <div>
                                  <p className="text-xs font-bold tracking-widest uppercase text-surface-300 dark:text-surface-500 mb-3">核心能力</p>
                                  <div className="space-y-2.5">
                                    {featuresArray.map((feature: string, idx: number) => (
                                      ...
                                      >
                                        ...
                                        <span className="text-sm text-surface-600 dark:text-surface-300 font-medium">{feature}</span>
                                      </div>
                                    ))}
                                  </div>
                                </div>
                              ...
                              >
                                <Link
                                  href={`/solutions/${solution.slug || slugify(solution.title || defaults.title || '')}`}
                                  className="inline-flex items-center gap-2 text-sm font-semibold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors group/btn"
                                  aria-label="查看完整方案"
                                >
                                  查看完整方案
                            ...
                            >
                              {solution.image ? (
                                <div className="w-full aspect-[4/3] overflow-hidden rounded-xl bg-surface-50 dark:bg-surface-900">
                                  <img
