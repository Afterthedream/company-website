      ...
      <section className="py-28 bg-surface-50 dark:bg-surface-950">
        <div className="max-w-6xl mx-auto px-6">
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            {/* 图片 */}
            <div className="relative rounded-2xl overflow-hidden bg-surface-100 dark:bg-surface-900 aspect-[4/3]">
              {companyImage ? (
                <Image src={companyImage} alt={data.name} fill className="object-cover" unoptimized />
              ) : null}
              <div className="absolute -bottom-3 -left-3 w-full h-full rounded-2xl border border-primary-100 dark:border-primary-900 -z-10" />
            </div>
            ...
            <div className="space-y-8">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-xs text-surface-500 dark:text-surface-400">公司简介</span>
                </div>
                <h2 className="section-title dark:text-surface-100">{data.name}</h2>
                <p className="text-surface-500 dark:text-surface-400 leading-relaxed text-[15px]">{data.description}</p>
              </div>

              <div className="flex gap-10">
                <div>
                  <div className="font-display text-3xl font-bold text-primary-600">{data.establishedYear}</div>
                  <div className="text-xs text-surface-500 dark:text-surface-400 mt-1">成立年份</div>
                </div>
                <div>
                  <div className="font-display text-3xl font-bold text-primary-600">{data.projectCount}+</div>
                  <div className="text-xs text-surface-500 dark:text-surface-400 mt-1">成功案例</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            <div className="flex items-center justify-center gap-3 mb-4">
              <div className="w-8 h-px bg-primary-200 dark:bg-primary-800" />
              <span className="text-xs font-semibold text-primary-600 tracking-widest uppercase">企业文化</span>
              <div className="w-8 h-px bg-primary-200 dark:bg-primary-800" />
            </div>
            <h2 className="section-title dark:text-surface-100">我们的理念</h2>
          </div>

          <div className="grid md:grid-cols-3 gap-5">
            {culture.map((item, i) => (
              <div key={i} className="card-surface dark:card-surface-dark p-8 card-hover">
                <div className="w-11 h-11 rounded-xl bg-primary-50 dark:bg-primary-900 flex items-center justify-center text-primary-600 dark:text-primary-400 mb-5">
                  {item.icon}
                </div>
                <span className="text-[11px] font-mono text-surface-300 dark:text-surface-600 block mb-2">0{i + 1}</span>
                <h3 className="text-lg font-semibold text-surface-900 dark:text-surface-100 mb-2">{item.title}</h3>
                <p className="text-sm text-surface-500 dark:text-surface-300 leading-relaxed">{item.text}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="py-28 bg-surface-50 dark:bg-surface-950">
        <div className="max-w-6xl mx-auto px-6">
          <div className="text-center mb-14">
            ...
            <h2 className="section-title dark:text-surface-100">公司信息</h2>
          </div>
          ...
          <div className="grid lg:grid-cols-2 gap-12">
            ...
            <div className="flex flex-col justify-between min-h-[360px]">
              {[
                ...
                <div key={i} className="flex items-start gap-4 p-5 rounded-xl bg-surface-50 dark:bg-surface-900 hover:bg-surface-100/80 dark:hover:bg-surface-800 transition-colors duration-200">
                  ...
                  <div>
                    ...
                    {item.href ? (
                      <a href={item.href} className="text-sm font-medium text-surface-800 dark:text-surface-200 hover:text-primary-600 transition-colors duration-200">{item.value}</a>
                    ) : (
