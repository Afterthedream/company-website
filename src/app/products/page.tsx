...
export default function ProductsPage() {
        ...
        if (productsData && productsData.length > 0) {
          ...
          const formattedProducts = productsData.map((product: any) => {
            ...
            return {
              ...
              features: product.features ? (Array.isArray(product.features) ? product.features : product.features.split(',')) : [],
  ...
  const displayProducts = selectedCategory
    ? products.filter((p) => p.category?.documentId === selectedCategory || p.category?.id === selectedCategory)
    : products
            ...
            <div className="flex flex-wrap justify-center gap-4 mb-16">
              ...
              {categories.map((category: any) => (
                <button
                  ...
                  onClick={() => setSelectedCategory(category.documentId || String(category.id))}
                  className={`px-6 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 micro-interaction ${
                    selectedCategory === (category.documentId || String(category.id))
                      ? 'bg-primary-600 text-white shadow-lg'
                      : 'bg-white text-surface-700 hover:bg-surface-100 border border-surface-200'
                  }`}
                >
              ...
              >
                  ...
                  <div className="p-6 space-y-4">
                    <div className="flex items-center justify-between">
                      <span className="text-[11px] font-mono text-surface-300">{String(index + 1).padStart(2, '0')}</span>
